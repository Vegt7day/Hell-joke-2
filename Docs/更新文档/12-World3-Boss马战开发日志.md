...
本日志用于**按时间记录** Boss 战实现中的操作、决策与待办，与设计对照文档 [`11-World3-Boss马战设计.md`](11-World3-Boss马战设计.md) 配合使用。
...
以下为**截至当前实现**仍建议推进、联调或回归验收的事项；与 [`11-World3-Boss马战设计.md`](11-World3-Boss马战设计.md) §10 检查清单、§11 里程碑对照使用。完成某项后可将对应条目勾掉或删行。
...
- [x] 音频：[`10-音频总线配置建议.md`](10-音频总线配置建议.md) 与 Boss 战 BGM；机关仍走 `MechanismSfxBus`（见 [`09-机关音频配置检查清单.md`](09-机关音频配置检查清单.md)）。

### 测试与文档

- [ ] 关键路径录屏或 checklist：入场 → 各阈值换马 → 20% 双分支（商鞅 / 无商鞅）→ 血量归零 `重拾五肢`（若有）。
- [ ] 将本清单中已完成项同步回 `11` §10 勾选或迁移至「变更记录」日期行。
