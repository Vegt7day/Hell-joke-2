...
A dummy `TextServer<class_TextServer>`{.interpreted-text role="ref"} interface that doesn\'t do anything. Useful for freeing up memory when rendering text is not needed, as text servers are resource-intensive. It can also be used for performance comparisons in complex GUIs to check the impact of text rendering.
...
The command line argument `--text-driver Dummy` (case-sensitive) can be used to force the \"Dummy\" `TextServer<class_TextServer>`{.interpreted-text role="ref"} on any project.
