...
const STANDARD_IMPULSE_MULTIPLIER: float = 2.0
const GREY_DASH_IMPULSE_MULTIPLIER: float = 4.0

@export var bump_dir: Vector2 = Vector2(-1.0, -0.42)
@export var bump_speed: float = 268.0
@export var bump_impulse_multiplier: float = STANDARD_IMPULSE_MULTIPLIER
## 离开该区域后才可以再次触发一次击飞
...
func _on_body_entered(body: Node2D) -> void:
	...
	if not p.has_method("apply_bump_from_horse"):
		return
	var impulse := bump_dir.normalized() * bump_speed * bump_impulse_multiplier
...
func apply_standard_bump_preset() -> void:
	...
	bump_impulse_multiplier = STANDARD_IMPULSE_MULTIPLIER
